...
**  18.10.1993  Fehlerkontrolle beim Schreiben in Ausgabefile
**  19.10.1993  Meldungen/Hilfstexte auf englisch
*/
...
static const char manual [] =
  "GOULD         February 1993    U.Berntien/P.Roewekamp\n"
  "COMMUNICATION WITH DIGITAL SCOPE GOULD VIA SERIAL PORT\n"
  "command format:\n\n"
  "   GOULD [-t<timeout>] [-p<port>] <baudrate>\n\n"
  "<baudrate> = integer (e.g. 9600)\n"
  "<timeout>  = maximum time for timeout (in tenth of seconds)\n"
  "             default value: 10 (1 sec)\n"
  "<port>     = number of port (1..4)"
  "             default value: 1\n\n";
...
static const char kommandos [] =
  "the transmission of data will be stopped by pressing the 'S'-key\n"
  "the program is terminated by an empty command line\n"
  "beside commands for the scope, also commands for the program are\n"
  "allowed; in this case the starting character is a point '.'; for\n"
  "command lines to the scope beginning with a point, the starting\n"
  "charcter must be sent twice '..'\n"
  ".help                    printing this text\n"
  ".exec <filename>         executing commands from a file,\n"
  "                         remark lines start with the character '%'\n"
  ".output <filename>       starting output to file\n"
  ".outputnew <filename>    like .output, the file must not exist\n"
  ".outputappend <filename> like .output, appending to existing file\n"
  ".output                  stopping output to file\n"
  ".port <nummer>           changing of serial port (1..4)\n"
  ".timeout <1/10sec>       changing time for timeout\n"
  ".reset                   initializing of serial port of the PC\n"
  ".system <befehl>         transmitting command to DOS\n"
  ".system                  starting command line interpreter\n";
...
**  msg : zusaetzliche Meldung oder NULL
*/
static void LOCAL fileerror ( const char* msg )
  {
...
static void LOCAL init ( int baud )
  ...
  if( rs232setparameter( &parameter ) )
    error( "CONFIGURATION OF PORT FAILED" );
  else
...
static const char* LOCAL transfer ( const char* cmd )
  ...
  if( *out == NULL )
	...
	puts( "received without errors" );
      else
	printf( "received with %d parity- and %d overflow-errors\n",
...
**  return : soll Übertragung unterbrochen werden ?
**           (durch Benutzer oder Fehler bei der Ausgabe in dtaout)
*/
static int LOCAL exec ( char* cmd )
  ...
  if( response != NULL )
    {
    result = fputs( response, dtaout ) == EOF;
    if( result ) fileerror( "output in file raise" );
    }
  else
    {
...
static char* fgetcmd (  FILE* file )
  ...
  if( akku ==  EOF )
    ...
    if( i >= buffer.size )
      ...
      {
      fileerror( "trouble by reading command file" );
      result = NULL;
      }
    else if( i < 2 )
      result = NULL;
...
/*
**  Befehle aus einer Datei ausfuehren
...
static void LOCAL batch ( const char* fname )
  ...
  if( input == NULL )
    {
    fprintf( stderr, "cannot open command file '%s'", fname );
    fileerror( NULL );
    }
  else
    {
    ...
    if( cmd != NULL ) fputs( "command file processing aborted\n", stderr );
    }
  fclose( input );
  }
...
static void LOCAL dtareset ( void )
  ...
  if( dtaout != stdout )
    {
    if( fclose( dtaout ) != 0 ) fileerror( "closing output file raise" );
    dtaout = stdout;
    }
  }
...
static void LOCAL dtaredirect ( const char* fname, char mode )
  ...
  if( ok )
    ...
    if( file != NULL )
      ...
      {
      fprintf( stderr, "cannot %s file '%s'."
	       ,mode == 'a' ? "open" : "create"
	       ,fname );
      fileerror( NULL );
      }
...
static void LOCAL setcom ( const char* nummer )
  ...
  if( com < 1 || com > 4 )
    {
    fprintf( stderr, "serial port: %s", nummer );
...
static void LOCAL settimeout ( const char* time )
  ...
  if( akku < 1 )
    ...
    error( "wait time at least 1/10 second" );
    }
  else
